...
class SLS {
		...
		matrix __tmp;
		value_type_pointer __vtmp = nullptr;

		// Main A matrix:
		matrix_reference __A;

		// b matrix:
		value_type_pointer __b;

		// Matrix A as vector - only for diagonal A:
		value_type_pointer __Ad;

		// 3 vectors for Thomas' algorithm:
		value_type_pointer __u;
		value_type_pointer __d;
		value_type_pointer __l;
		long __n;
	...
	public:
		...
		explicit SLS(matrix_reference A, value_type_pointer b) : __A{ A }, __b{b}, __l{ __vtmp }, __d{ __vtmp }, __u{ __vtmp }, __n{ A.getRows() }, __Ad{ __vtmp } {
			if(!is_square(A))
		...
		explicit SLS(value_type_pointer A, value_type_pointer b, long n) : __Ad{ A }, __b{ b }, __A{ __tmp }, __l{ __vtmp }, __d{ __vtmp }, __u{ __vtmp }, __n{ n } {
			//if (A.size() != b.size())
			//	throw *(new SLSException("SLS::SLS(): Matrix A must have the same amount of rows as matrix b"));
		}


		explicit SLS(value_type_pointer l, value_type_pointer d, value_type_pointer u, long n) : __l{ l }, __d{ d }, __u{ u }, __n{ n }, __A { __tmp }, __b{ __vtmp }, __Ad{ __vtmp } { }
		...
		*/
		...
		self_reference setA(value_type_pointer A) {
			//if (A.empty())
		...
		self_reference setb(value_type_pointer b) {
		...
		value_type_pointer getSolutions(void) {
			return __b;
		}// end getSolutions
};
